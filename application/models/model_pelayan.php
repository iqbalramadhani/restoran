...
class Model_pelayan extends CI_Model {
    ...
    function tampil_pesanan(){
        $query = "SELECT pd.id_pesanan,pl.nama,GROUP_CONCAT(pd.jumlah) as jumlah,GROUP_CONCAT(m.nama) as nama_pesanan 
                  ...
                  WHERE pd.id_menu = m.id_menu AND pd.id_pesanan = p.id_pesanan AND p.id_pelanggan = pl.id_pelanggan AND p.status_pesanan = '1' AND p.status_antar = '0'  
    ...
    function antar($id){
        ...
        $this->db->update('pesanan',array('status_antar'=>'1'));
    }
}